        ...
        result.forEach((item) => {
            container.prepend(
                '<article class="post vt-post">' +
                    ...
                    '<div class="caption">' +
                    '<h3 class="md-heading"><a href="http://127.0.0.1:8000/detail/' +
                    item.id +
                    '">' +
                    item.title +
                    "</a></h3>" +
                    ...
                    item.content +
                    "</p>" +
                    '<a class="btn btn-success edit_btn" href="#editArticleModal" data-toggle="modal" role="button" style="margin-right:6px" data-id="' +
                    item.id +
                    '">Edit</a>' +
                    '<a class="btn btn-danger delete_btn" href="#deleteArticleModal" data-toggle="modal" role="button" data-id="' +
                    item.id +
                    '">Delete</a>  </div>' +
                    "</div>" +
                    "</div>" +
...
$("#updateArticle").click(function (e) {
    e.preventDefault();
    const formData = $("#editFormArticle :input")
